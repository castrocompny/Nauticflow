...
**Decisão final (pedido do dono):** tirar todo o texto de marketing da imagem — sem headline, subtexto, botão nem domínio — e deixar **só a logo** (ícone + "NauticFlow") grande e centralizada (tanto horizontal quanto verticalmente) sobre o fundo em gradiente da marca. `public/og-image.png` gerado via `sharp` (script único, não fica no repo): fundo é um gradiente diagonal reconstruído a partir das mesmas 4 cores de canto do design original (interpolação bilinear), com a logo (extraída do design antigo, fundo removido via chroma-key pela cor real do gradiente local, não uma cor fixa) redimensionada e centralizada por cima. Validado simulando o recorte quadrado central (630×630) que o WhatsApp faz na versão compacta: a logo cabe inteira, com folga, nos dois formatos.
