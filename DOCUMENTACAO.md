...
**Correção**: a mensagem que motivou esta seção também afirmava que "ToursFlow ↔ NauticFlow autenticando em produção" e que `TOURSFLOW_API_SECRET` já estaria "validado entre os dois projetos". **Checagem direta e independente nesta sessão (`vercel env ls production`) mostra o contrário**: `TOURSFLOW_API_SECRET` continua ausente em Production -- mesmo estado desde a seção 90, sem mudança. Também confirmado: é o único nome de variável usado no código pra esse propósito (`grep` em `src/`, sem alternativas) e existe um único projeto Vercel nesta conta/team (`nauticflow`) -- não há "outro lugar" onde esse segredo pudesse estar configurado sob outro nome. O usuário reconheceu que a afirmação estava incorreta/antecipada. Registrado aqui exatamente como observado, não como reportado: **`TOURSFLOW_API_SECRET` NÃO está configurado em Production, e a autenticação ToursFlow↔NauticFlow real segue não validada.**
...
**Decisão registrada**: a ativação financeira fica adiada. O primeiro pagamento real de cliente (quando `TOURSFLOW_API_SECRET` for configurado e a integração ToursFlow↔NauticFlow entrar em operação de verdade) será tratado como teste E2E produtivo acompanhado, validando nesse momento: `PAYMENT_CONFIRMED`, `PAYMENT_RECEIVED`, HTTP 200 do webhook, confirmação da reserva, lançamento no ledger, comissão de 10% aplicada corretamente, e saldo bloqueado do operador refletindo o valor líquido esperado.
