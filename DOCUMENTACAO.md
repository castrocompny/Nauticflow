...
**Correção:** recentralizada a logo (ícone + "NauticFlow") horizontalmente na imagem, sem mudar o resto do design. Feito via `sharp` (script único, não fica no repo): recorta o bloco original do logo, reconstrói o fundo (gradiente diagonal) por trás dele via interpolação bilinear a partir de amostras de pixel reais próximas — pra "apagar" o logo antigo sem deixar emenda visível — e recompõe o logo (com fundo removido via chroma-key baseado na cor real do gradiente local, não uma cor fixa) centralizado no topo. Validado simulando o recorte quadrado central (630×630) que o WhatsApp faz: a logo aparece por completo nele agora.
