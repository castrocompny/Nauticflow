...
- **Senha mais forte no cadastro/reset** — pedido do dono do produto (2026-08-14). Hoje `signUp` em `login/actions.ts` não valida força de senha nenhuma, só usa o mínimo padrão do Supabase Auth (6 caracteres, sem exigência de maiúscula/número/símbolo). Adicionar validação (client e/ou server) antes de liberar cadastro.
- **Botão de excluir a própria conta** — pedido do dono do produto (2026-08-14). Hoje só existe remoção de outros membros da equipe (`removeTeamMember`, seção 8) — não existe um fluxo de auto-exclusão de conta/empresa (relevante inclusive pra LGPD, direito ao esquecimento). Precisa decidir o que "excluir conta" apaga de fato (só o usuário? a empresa inteira com todos os dados? período de carência antes de apagar de vez?) antes de implementar.
- **Chat de suporte online** — pedido do dono do produto (2026-08-14). Hoje o único contato de suporte é o link de WhatsApp no `OverdueBanner` (`src/app/(app)/overdue-banner.tsx`). Considerar um widget de chat ao vivo (Crisp, Tawk.to, Intercom etc.) — precisa decidir qual provedor antes de integrar.
