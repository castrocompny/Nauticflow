...
**Chat de suporte** (`src/app/layout.tsx`): widget do Tawk.to (escolhido por ser gratuito sem limite de atendente/conversa), carregado via `next/script` com `strategy="lazyOnload"` (não compete com o carregamento/hidratação da página) direto no `<body>` do layout raiz — aparece em todas as páginas, incluindo `/login` (visitante não logado também consegue tirar dúvida antes de criar conta). Property ID e Widget ID do Tawk.to (`6a80e6c138c19e1d457853de` / `1k03oapev`) estão hardcoded no script — não são segredo (ficam expostos no HTML de qualquer forma, é um widget client-side), então não há necessidade de variável de ambiente. Conversas são respondidas pelo painel em `dashboard.tawk.to` (tem app mobile).
