...
- 🔴 **PRIORIDADE — confirmar se o Asaas em produção está mesmo em modo produção (não sandbox), e cancelar a assinatura da própria empresa do dono** (sessão de 2026-08-18, atualizado em 2026-08-19): a empresa "Castro Compny" (a do próprio dono/super admin, `company_id` `a1c92771-36d7-43f7-8898-27cac01b8cb2`) tem `asaas_subscription_id` = `sub_klqdrpcxesksrn3c` guardado no nosso banco — mudar o `paid_until` no nosso sistema (ver seção 33) não cancela isso no Asaas, são coisas separadas.
  - **Descoberta em 2026-08-19**: o dono do produto conferiu a conta de produção do Asaas e ela está **vazia** (nenhuma assinatura) — as 3 assinaturas que ele encontrou e cancelou (`castro compny` R$297 Profissional + 2× `Minha empresa` R$597 Premium, essa última duplicada) eram todas do ambiente **Sandbox** (teste), não de produção. Isso levanta a dúvida se o sistema em produção (`nauticflow.com.br`) está configurado pra usar a API de **produção** do Asaas ou se, por engano, está configurado com a URL/chave do **Sandbox** — o que explicaria a conta de produção vazia e por que a chave salva localmente (`.env.local`) deu "chave inválida" quando testada.
  - Não foi possível confirmar por aqui: as variáveis `ASAAS_API_URL`/`ASAAS_API_KEY` na Vercel estão marcadas como "Sensitive" e a Vercel não revela o valor real nem via `vercel env pull` pra quem não é o dono da conta (mostra só `[SENSITIVE]`).
  - **Próximo passo, quando o João puder**: ele entra em vercel.com → projeto `nauticflow` → Settings → Environment Variables → clica no ícone de olho em `ASAAS_API_URL` (e confirma se contém "sandbox" ou não) e em `ASAAS_API_KEY` (chave de sandbox costuma começar diferente da de produção — conferir no próprio painel do Asaas, em Integrações → Chaves de API, qual chave é de qual ambiente). Se confirmar que está em sandbox, precisa trocar pra chave/URL de produção de verdade — **isso significa que nenhum pagamento real está sendo processado hoje**, o que é uma descoberta maior que só cancelar uma assinatura de teste.
  - Se, ao contrário, confirmar que já está em produção e a conta realmente está vazia, então não tem nada pra cancelar — a pendência se resolve sozinha (o `asaas_subscription_id` salvo no banco seria de um teste antigo feito em sandbox, e pode ser ignorado/limpo).
