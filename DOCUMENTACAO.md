...
- **Aplicar a migration `0019_valida_dono_fk_saidas.sql`** no SQL Editor do Supabase — código já corrigido (seção 31), falta só rodar a migration.
- **Rate limiting/CAPTCHA no login** — confirmar limites reais no painel do Supabase Auth antes de abrir ao público; considerar hCaptcha (ver seção 31).
- **Validar valor da reserva (`total_cents`) contra o preço do passeio** — hoje aceita qualquer valor do formulário sem checagem (ver seção 31).
- **Avaliar migração major do Next.js** — 5 advisories HIGH no `npm audit`, correção exige `next@16` (ver seção 31).
...
1. **🔴 IDOR em `departures.vessel_id`/`tour_id` — corrigido nesta sessão.** A migration 0015 (seção 13) tinha corrigido o mesmo tipo de falha em `reservations`/`passengers`, mas esqueceu `departures`: `createDeparture`/`updateDeparture` (`saidas/actions.ts`) aceitavam `vessel_id`/`tour_id` do formulário sem checar se pertenciam à própria empresa — nem no app, nem no banco. Um usuário autenticado de qualquer empresa que soubesse o UUID de uma embarcação/passeio de outra empresa podia criar uma saída referenciando ela. **Corrigido em duas camadas**, mesmo padrão da 0015: checagem explícita de `company_id` no app (`saidas/actions.ts`, `createDeparture` e `updateDeparture`) + gatilho novo no banco, `supabase/migrations/0019_valida_dono_fk_saidas.sql` (**precisa ser aplicada manualmente no SQL Editor do Supabase**, como todas as migrations deste projeto — ver seção 9).
2. **🟠 Rate limiting/CAPTCHA no login** — não implementado no app, depende inteiramente do limite padrão do Supabase Auth (não visível/configurável pelo código). Pendente de decisão: confirmar limites reais no painel Supabase e/ou adicionar hCaptcha antes de abrir ao público.
3. **🟡 Valor da reserva (`total_cents`) sem validação contra `tours.base_price_cents`** — qualquer `staff` autenticado pode gravar um valor arbitrário (inclusive negativo) numa reserva, via `reservas/actions.ts`. Não é uma falha de pagamento externo (não há checkout de cliente final), mas é uma brecha de integridade que afeta a confiabilidade do Financeiro/Dashboard e abre espaço pra fraude interna. Pendente.
