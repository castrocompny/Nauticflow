...
- **Landing: contato real** — trocar o WhatsApp/e-mail placeholder em `src/components/marketing/plans.ts` (`MKT_CONTACT`, hoje `(00) 00000-0000` / `contato@nauticflow.com.br`) antes de divulgar o site institucional (ver seção 40).
- **Landing: depoimentos** — a seção "Confiança" da landing tem um espaço reservado, marcado como placeholder; preencher com avaliações reais de clientes (nada inventado) antes de divulgar (ver seção 40).
