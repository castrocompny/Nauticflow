...
`feature/marketplace-payments` (criada a partir de `453e079`) e o hardening de segurança (seção 74, commits `e1d90ae`/`571f57a`, pushados direto pra `main`) avançaram em paralelo, sem saber um do outro -- a colisão de numeração de migrations já tratada na seção anterior (renumerada pra 79 nesta reorganização, ver abaixo) foi um sintoma disso. Depois de fechar a revisão financeira da Fase 4B, esta sessão reconciliou a branch com o `main` atualizado antes de qualquer commit novo.
