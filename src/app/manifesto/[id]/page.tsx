  ...
  if (!data) {
    return (
      <div className="mx-auto max-w-2xl p-8">
        <Link href="/saidas" className="inline-flex items-center gap-1.5 text-sm text-slate-500">
  ...
  return (
    <div className="mx-auto max-w-4xl p-8 print:p-0">
      <div className="no-print mb-6 flex items-center justify-between">
        <Link href={`/saidas/${d.id}`} className="inline-flex items-center gap-1.5 text-sm text-slate-500">
          <ArrowLeft size={16} /> Voltar à saída
        </Link>
        <PrintButton />
      </div>

      <div className="rounded-card border border-slate-200 bg-white p-8 print:border-0 print:p-0">
        {/* Cabecalho */}
        <div className="mb-6 flex items-start justify-between border-b border-slate-200 pb-4">
          <div>
            ...
            <h1 className="font-display text-2xl font-semibold text-navy">Manifesto de passageiros</h1>
            <p className="text-sm text-slate-500">{d.companies?.name}</p>
          </div>
          <div className="text-right text-sm">
            <p className="font-medium text-navy">{d.vessels?.name}</p>
        ...
        {reservas.length === 0 ? (
          <div className="rounded-lg bg-slate-50 py-10 text-center text-sm text-slate-500">
          ...
          <div className="space-y-4">
            {reservas.map((r, idx) => (
              <div key={r.id} className="rounded-lg border border-slate-200">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-50 px-4 py-2.5 print:bg-white">
                  <div>
                    ...
                    <tbody>
                      {r.passengers.map((p, i) => (
                        <tr key={i} className="border-t border-slate-50">
                          <td className="px-4 py-1.5 text-navy">{p.name}</td>
                          <td className="px-4 py-1.5 text-slate-600">{p.document ?? "-"}</td>
                          <td className="px-4 py-1.5 text-slate-600">{p.nationality ?? "-"}</td>
                          <td className="px-4 py-1.5 text-right text-slate-600">{statusLabel[p.status] ?? p.status}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                {r.notes && <p className="border-t border-slate-100 px-4 py-2 text-xs text-slate-500">Obs.: {r.notes}</p>}
              </div>
            ))}
          </div>
        )}
        ...
        <div className="mt-10 grid grid-cols-2 gap-10 text-sm">
          <div>
            <div className="border-t border-slate-400 pt-1 text-center text-slate-500">Conferido por</div>
          </div>
          <div>
            <div className="border-t border-slate-400 pt-1 text-center text-slate-500">Comandante</div>
          </div>
        </div>
