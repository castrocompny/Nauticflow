  ...
  return (
    <>
      ...
      <div className="mb-4 flex gap-2">
        <Link
          ...
          className={`rounded-lg border px-3 py-1.5 text-sm transition ${
            p === "mes" ? "border-brand bg-brand text-white" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
          }`}
        >
          Receita do mês
        </Link>
        <Link
          ...
          className={`rounded-lg border px-3 py-1.5 text-sm transition ${
            p === "ano" ? "border-brand bg-brand text-white" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
          }`}
        >
          Receita do ano
        </Link>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Stat label="Ticket médio" value={brl(ticket)} tone="text-navy" />
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Stat label={`Total de reservas ${periodoLabel}`} value={String(rows.length)} tone="text-navy" />
      </div>

      <Card className="p-0">
        <h2 className="px-5 py-4 font-display text-base font-semibold text-navy">Movimentações {periodoLabel}</h2>
        {rows.length === 0 ? (
          <p className="px-5 pb-6 text-center text-sm text-slate-400">Nenhuma movimentação {periodoLabel}.</p>
        ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-slate-50 last:border-0">
                  ...
                  <td className="px-4 py-3 text-slate-600">{r.departures?.vessels?.name ?? "-"}</td>
