...
function buildEmail(r: any): string {
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden">
    <div style="background:#0d1b3e;padding:20px 24px;color:#fff">
      <span style="font-size:18px;font-weight:700">Nautic<span style="color:#2d9cff">Flow</span></span>
      <div style="font-size:12px;color:#cbd5e1;margin-top:2px">${r.companies?.name ?? ""}</div>
    </div>
    <div style="padding:24px">
      <p style="font-size:15px;color:#0d1b3e">Olá, ${r.clients?.name ?? "cliente"}!</p>
      ...
      <table style="width:100%;border-collapse:collapse">
        ${row("Passeio", dep.tours?.name ?? "-")}
        ${row("Embarcação", dep.vessels?.name ?? "-")}
        ${row("Data", date)}
        ${row("Horário", time)}
